...
type loadFileResult = {
    filename: string,
    response: Buffer | ArrayBuffer | string | ResponseDataDetailed<BufferLike | string> | any,
}
...
export const MultiFileEditor = ({emitter}: Props) => {
    ...
    const getEncodingJs = (data:any) => {
        const isUtf8 = isUTF8Encoded(data);
        return isUtf8 ? 'UTF-8' : null;
      };
      
      // Function to check if the data is UTF-8 encoded
      const isUTF8Encoded = (data:any) => {
        let i = 0;
        while (i < data.length) {
    ...
    useEffect(() => {
        const handleFileLoaded = (data: loadFileResult | any) => {
            const qbuffer = Buffer.from(data.response);

            const encoding:any = getEncodingJs(qbuffer)
